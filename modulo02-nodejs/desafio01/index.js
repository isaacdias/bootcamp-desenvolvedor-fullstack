...
async function getBiggerOrSmallerCityName(bigger) {
  ...
  for (state of states) {
    ...
    list.push({ nome: city.Nome, uf: state.Sigla });
  }
  const result = list.reduce((prev, current) => {
    if (bigger) {
      if (prev.nome.length > current.nome.length) return prev;
      else if (prev.nome.length < current.nome.length) return current;
      else
        return prev.nome.toLowerCase() < current.nome.toLowerCase()
          ? prev
          : current;
    } else {
      if (prev.nome.length < current.nome.length) return prev;
